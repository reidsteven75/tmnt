...
class App extends Component {
	...
	parserStart() {
		...
		this.cache.parserFunction = setInterval(() => {
			...
			if (typeof steps[parseIndex] !== 'undefined') {
				stepCurrent = steps[parseIndex]
			}
			if (typeof steps[parseIndex + 1] !== 'undefined') {
				stepNext = steps[parseIndex + 1]
			}
			if (typeof steps[parseIndex - 1] !== 'undefined') {
				stepPrevious = steps[parseIndex - 1]
			}
	...
	_handleLoadClicked() {
		this.setState({
			...
			try {
				...
				if (roundedGridDim <= 50) {
					viewerZoomConfig = config.zoom['min']
				} else if (roundedGridDim > 50 <= 80) {
					viewerZoomConfig = config.zoom['mid']
				} else if (roundedGridDim > 80) {
					viewerZoomConfig = config.zoom['max']
				}
	...
	_handleAnimateClicked() {
		...
		if (isAnimate !== true) {
			...
			if (stepCount + 1 === parseIndex) {
				newState.parseIndex = 0
			}
			this.parserStop()
	...
	render() {
		...
		return ( <
			...
			handleAnimateClicked = {
				this._handleAnimateClicked
			}
			handleSpeedChange = {
				this._handleSpeedChange
			}
			handleLoadClicked = {
				this._handleLoadClicked
			}
			/> <
			/Drawer> <
			main style = {
			...
			handleSliderChange = {
				this._handleViewerSliderChange
			}
			handleRendererUpdateDimensions = {
				this._handleRendererUpdateDimensions
			}
			/> <
			/main>
